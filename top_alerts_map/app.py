...
def server(input, output, session):
    @reactive.calc
    def full_data():
        return pd.read_csv("top_alerts_map/top_10_alerts.csv")
    ...
    def filtered_data():
        ...
        return df[df["type_subtype"] == selected_combination]
    ...
    def scatter_plot():
        ...
        scatter_chart = alt.Chart(filtered, title='Top 10 Locations for Heavy Traffic Jams').mark_circle().encode(
        ...
        combined_chart = (map_chart + scatter_chart).configure_view(
            ...
            title=f"Chicago Neighborhood Map with {input.type_subtype()} Observations"  # Use f-string to combine all parts into the title
        ).configure_mark(
            opacity=0.7,
            color='darkblue'
        )
